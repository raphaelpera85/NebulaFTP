...
class NebulaGUI:
    ...
    def _create_widgets(self):
        ...
        self.path_var = tk.StringVar(value=os.environ.get("MONITOR_PATH", "E:\\"))
        ...
        browse_btn = tk.Button(entry_btn_frame, text="📂 Selecionar Pasta", command=self._browse_folder, font=("Segoe UI", 9, "bold"), bg="#45475a", fg="white", bd=0, padx=12, pady=4, cursor="hand2")
    ...
    def start_services(self):
        ...
        if not os.path.exists(source):
            messagebox.showerror("Erro", f"Caminho não encontrado: {source}")
            return
        ...
        self.log(f"Iniciando NebulaFTP Server e Monitor na pasta: {source}")
        ...
        self.proc_feed = subprocess.Popen(
            [sys.executable, "-u", "tools/feed_ftp.py", "--source", source, "--direct-mongo", "--workers", "2", "--watch", "--max-active", "60", "--poll-seconds", "60", "--delete-source"],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
            encoding="utf-8",
            ...
            if self.is_running:
                try:
                    ...
                    for doc in active_docs:
                        ...
                        info_text = f"👷 Worker #{worker_id} | 🤖 Bot #{bot_idx} | {name[:50]}" if status == "uploading" else f"⏳ Na Fila | {name[:50]}"
