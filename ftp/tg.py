...
class File:
    ...
    async def getChunkAt(self, offset=0):
        ...
        try:
            return (await session.send(GetFile(location=self.loc, offset=offset, limit=GETFILE_CHUNK_SIZE))).bytes
        except (TimeoutError, asyncio.TimeoutError):
            await asleep(1)
            return await self.getChunkAt(offset)
        except RPCError as exc:
            logger.debug("tg get chunk interrupted: %s", exc)
            return b""
    ...
    async def stream(self, offset=0):
        try:
            while data := await self.getChunkAt(offset):
                offset += len(data)
                yield data
                if len(data) != GETFILE_CHUNK_SIZE:
                    break
