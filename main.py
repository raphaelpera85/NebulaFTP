...
async def resolve_parent_to_path(mongo, parent, default_user_root: str) -> str:
    """Safely converts parent (whether string, ObjectId, None, etc.) to a valid string path."""
    if isinstance(parent, str):
        ...
        return "/" + p.strip("/")
    if parent is None:
        return f"{default_user_root}/Filmes"
    ...
    try:
        while curr and curr not in visited:
            ...
            if isinstance(curr, str):
                path_parts.insert(0, curr.strip("/"))
                break
            parent_doc = await mongo.files.find_one({"_id": curr})
            if not parent_doc:
                break
            name = parent_doc.get("name")
            if name:
                path_parts.insert(0, name)
            curr = parent_doc.get("parent")
    except Exception as e:
        logger.warning("Erro ao resolver parent ObjectId %s: %s", parent, e)

    if path_parts:
        full = "/" + "/".join(filter(None, path_parts))
        if not full.startswith(default_user_root):
            full = f"{default_user_root}{full}"
        return full
    return f"{default_user_root}/Filmes"
...
async def resolve_media_parent(mongo, parent, filename):
    ...
    if not isinstance(parent, str):
        parent = await resolve_parent_to_path(mongo, parent, default_user_root)
...
async def restore_pending_uploads(mongo):
    ...
    for doc in pending:
        try:
            ...
            logger.error("Erro ao restaurar arquivo pendente %s: %s", doc.get("name"), exc)
        ...
        try:
            ...
            for _ in range(max_workers):
                ...
                try:
                    ...
                    logger.error("Scanner Mongo: erro processando arquivo %s: %s", doc.get("name"), doc_exc)
            if found:
