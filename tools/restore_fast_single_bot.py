...
async def main():
    ...
    print(f"=== Restauração Sequencial Rápida Telegram -> MongoDB ===")
    print(f"Mídias a associar: {len(media_items)}")
    print("=========================================================\n")

    app = Client("recon_single_bot", api_id=api_id, api_hash=api_hash, bot_token=bot_tokens[0], in_memory=True)
    obfuscated_dict = {}
        ...
        for b_start in range(1, total_max_id, batch_size):
            ...
            for attempt in range(3):
                try:
                    ...
                    print(f"FloodWait {fw.value}s no msg_id {b_start}...")
            ...
            if b_start % 10000 == 1:
                print(f"[PROGRESSO] Escaneados até msg_id={b_start}/110000 ({len(obfuscated_dict)} mídias encontradas)")

            await asyncio.sleep(0.05)

    sorted_obf = sorted(obfuscated_dict.values(), key=lambda x: x["min_msg_id"])
    print(f"\n[100%] Telegram escaneado: {len(sorted_obf)} mídias completas encontradas!")
    print("Atualizando referências de streaming reais no MongoDB...")

    now = int(time.time())
    user_root = "/raphael"

    restored_count = 0
    for idx, obf in enumerate(sorted_obf):
    ...
    print(f"\n[SUCESSO COMPLETO] {restored_count} mídias atualizadas no MongoDB com file_ids reais do Telegram!")
