...
def main():
    ...
    try:
        ...
        rclone_script = str(Path(__file__).resolve().parent / "start_rclone_z.ps1")
        if os.path.exists(rclone_script):
            subprocess.Popen(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", rclone_script])
            print("Unidade N: atualizada!")
        else:
            print("Script rclone não encontrado no caminho esperado.")
