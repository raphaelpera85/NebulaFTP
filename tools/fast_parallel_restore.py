...
async def scan_bot_chunk(bot_index, token, chat_id, api_id, api_hash, start_id, end_id, obfuscated_dict, lock):
    app = Client(f"par_bot_{bot_index}", api_id=api_id, api_hash=api_hash, bot_token=token, in_memory=True)
    batch_size = 100
    local_count = 0

    await asyncio.sleep((bot_index - 1) * 0.15)
    try:
        async with app:
        ...
        print(f"[BOT #{bot_index}] Exception: {exc}")

    print(f"[BOT #{bot_index}] Finished range {start_id}..{end_id} ({local_count} parts found)")
...
async def main():
    ...
    print(f"Total state media items to map: {len(ordered_state_media)}")
    ...
    print(f"Starting parallel Telegram scan across {len(bot_tokens)} bots (range 1..{total_max_id})...")
    obfuscated_dict = {}
    lock = asyncio.Lock()
    ...
    print(f"\nScan complete! Found {len(sorted_obf)} complete media objects in Telegram.")
    ...
    print(f"Restoring {limit} files to MongoDB with correct title & parent structure...")
    ...
    print(f"\n[DONE] Successfully restored {restored} files into MongoDB!")
