...
def main():
    ...
    print(f"Total probed items: {len(items)}, Live MongoDB docs: {len(docs)}")
    ...
    for item in items:
        ...
        # Check 1: SxxEyy episode match inside stream tags
            ...
            if len(normalized(show)) >= 2 and not GENERIC_TITLES.match(normalized(show)):
                ...
                if curr_parent != desired_parent or curr_name != desired_name:
                    realignments.append({
                        ...
                        "evidence": f"Series Stream Tag: {title_text}"
                    })
                    continue

        # Check 2: Movie (YYYY) match inside stream tags
            ...
            if len(normalized(movie)) >= 2 and not GENERIC_TITLES.match(normalized(movie)):
                ...
                if curr_parent != desired_parent or curr_name != desired_name:
                    realignments.append({
                        ...
                        "desired_parent": desired_parent,
                        "desired_name": desired_name,
                        "evidence": f"Movie Stream Tag: {title_text}"
                    })
                    continue

    print(f"\nMode: {'APPLY' if apply else 'DRY-RUN'}")
    print(f"Total stream-verified content realignments: {len(realignments)}")
    for r in realignments[:20]:
        print(f"{r['curr']} -> {r['desired_parent']}/{r['desired_name']}")
    ...
    print(f"\nSUCCESS: Applied {applied} moves, quarantined {quarantined} duplicates, cleaned {deleted_dirs} empty directories!")
